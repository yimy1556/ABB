...
abb_iter_t *abb_iter_in_crear( const abb_t *arbol ){
    cola_t* cola = cola_crear(); // estoy en duda si hay que verificar si se creo, sino, no se puede guardar
    recorrido_in_order( arbol->raiz , guardar_cola , cola );
    abb_iter_t* iter_abb = malloc( sizeof(abb_iter_t) );
    if( !iter_abb ) return NULL; // creo que si no se crea el iterador, la cola se destruye.
    iter_abb->cola = cola;
    return iter_abb;
    }
